...
async def get_articles_semantic(query: str, top_k: int = 20) -> list[ArticleModel]:
    query_vec = embedding_service.encode(query)  # уже есть в твоём embedding_service
    return await ArticleModel.get_articles_for_rag(query_vec, top_k=top_k)  # уже есть в db_service
...
async def process_and_save(raw_articles: list[dict]) -> dict:
    ...
    for article in clustered:
        ...
        article["entities"] = ner_service.extract_entities(text)
